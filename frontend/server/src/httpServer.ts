...
import { publishApprovedThread } from './services/threadsPublisher.js';
...
export async function createHttpServer(options: HttpServerOptions): Promise<HttpServerHandle> {
  ...
  registerThreadsPublishRoute(app);
...
function registerThreadsPublishRoute(app: FastifyInstance): void {
  app.post<{
    Body: {
      text?: string;
      approvedByOperator?: boolean;
    };
  }>('/api/threads/publish', async (request, reply) => {
    const { text, approvedByOperator } = request.body;
    ...
    if (approvedByOperator !== true) {
      reply.code(400).send({ ok: false, error: 'approvedByOperator must be true.' });
      return;
    }

    try {
      const result = await publishApprovedThread({
        draftText: text,
        approvedByOperator,
      });
