...
interface ToolOverlayProps {
  ...
  agentDialogues?: Record<number, string>;
  agentDepartments?: Record<number, 'marketing' | 'cs'>;
}
  ...
  return (
    <>
      {allIds.map((id) => {
        ...
        const displayActivityText = isTeamAgent && teamRoleLabel ? teamRoleLabel : activityText;
        const department = agentDepartments[id];
        const departmentColor =
          department === 'cs' ? '#27c7d9' : department === 'marketing' ? '#8b6cff' : undefined;
        const departmentBg =
          department === 'cs'
            ? 'rgba(20, 90, 104, 0.74)'
        ...
        const hasExtraLines = !isTeamAgent && !!(ch.folderName || teamRoleLabel);
        const dialogue = agentDialogues[id];
        ...
        return (
          ...
          >
            {dialogue && (
              <div className="mb-4 max-w-240 border-2 border-accent bg-bg-dark px-7 py-5 text-[10px] text-text leading-tight whitespace-normal shadow-pixel">
                {dialogue}
              </div>
            )}
            <div
              className="flex items-center border-border px-8 py-4 gap-5 pixel-panel whitespace-nowrap max-w-2xs"
              style={{
                borderColor: departmentColor,
                background: departmentBg,
              }}
            >
              {!isTeamAgent && dotColor && (
                <span
              ...
              <div className="flex flex-col gap-0 overflow-hidden">
                ...
                <span
                  className="overflow-hidden text-ellipsis block leading-none"
                  style={{
                    fontSize: isTeamAgent ? '16px' : isSub ? '20px' : '22px',
                    fontStyle: isSub ? 'italic' : undefined,
                    color:
                      isTeamAgent && ch.isTeamLead ? TEAM_LEAD_COLOR : departmentColor,
                    fontWeight: isTeamAgent && ch.isTeamLead ? 'bold' : undefined,
                  }}
                >
                  {displayActivityText}
                </span>
