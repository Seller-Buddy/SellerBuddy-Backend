...
function readStoredLayout(): unknown | null {
  try {
    const raw = window.localStorage.getItem(BROWSER_DEV_LAYOUT_STORAGE_KEY);
    return raw ? (JSON.parse(raw) as unknown) : null;
  } catch (err) {
    console.warn('[BrowserMock] Failed to read saved dev layout:', err);
    return null;
  }
}
